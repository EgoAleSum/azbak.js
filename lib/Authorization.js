...
class Authorization {
    ...
    setStorageAccount(name, key, sasToken) {
        ...
        let hasKey = !!(key && _.isString(key) && key.length > 1)
        let hasSasToken = !!(sasToken && _.isString(sasToken) && sasToken.length > 1)
    ...
    generateSignature() {
        ...
        let hmac = crypto.createHmac('sha256',
            new Buffer(this._storageAccountKey || this._storageAccountSasToken, 'base64')
        )
    ...
    querystring() {
        ...
        if(this._storageAccountSasToken) {
            ...
            let qsSas = qs.parse(this._storageAccountSasToken.substring(1))
