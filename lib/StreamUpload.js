...
class StreamUpload {
    ...
    constructor(sourceStream, blob, authData) {
        ...
        if(!authData
            ...
            || (!authData.storageAccountKey && !authData.storageAccountSasToken)) {
            throw Error('Storage account name and key or SAS token must be set')
        }
        ...
        // Blob url
        this._blobUrl = 'https://' + this._authData.storageAccountName + '.' + this.endpoint + blob
    }
