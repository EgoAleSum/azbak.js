...
class StreamUpload {
    ...
    upload() {
        ...
        return new Promise((resolve, reject) => {
            ...
            const transformFunction = through2Concurrent(
                ...
                (chunk, enc, callback) => {
                    ...
                    console.log('(Seq ' + seqId + ') Starting upload of block ' + blockNum + ', length ' + chunk.length + ', blockId ' + blockId)

                    this.putBlock(chunk, blockId, seqId)
                        .then((response, body) => {
                            console.log('(Seq ' + seqId + ') Finished upload of chunk ' + blockNum  + ', blockId ' + blockId, response.body)
                            callback()
                        })
                        .catch((error) => {
                            reject(error)
        ...
        .then((chunkCount) => {
            console.log('Uploaded total: ' + chunkCount)

            // Promises to return
            let promises = []

            // Count how many parts (sequences) we have
            let seqCount = Math.trunc(chunkCount /  blocksPerBlob) + ((chunkCount % blocksPerBlob) ? 1 : 0)
            ...
            for(let i = 0; i < seqCount; i++) {
                ...
                let blocksInSeq = (i === (seqCount - 1)) ? (chunkCount % blocksPerBlob) : blocksPerBlob

                promises.push(this.commitBlockBlob(blocksInSeq, seqId))
            }
